...
x_original, y_original, z_original = Lorenz(x0 = x0, y0 = y0, z0 = z0, sigma = sigma, beta = beta, rho = rho, num_steps = num_steps)
x = x_original + np.random.normal(loc=0, scale=np.std(x_original)*l_noise, size=x_original.shape)
y = y_original + np.random.normal(loc=0, scale=np.std(y_original)*l_noise, size=y_original.shape)
z = z_original + np.random.normal(loc=0, scale=np.std(z_original)*l_noise, size=z_original.shape)
...
NDEI = RMSE/st.stdev(np.asarray(y_test.flatten(), dtype=float))
print("NDEI:", NDEI)
...
def build_model(n_hidden=0, n_neurons=0, activation = "relu", learning_rate=0.474, input_shape=[4]):
    model = Sequential()
...
best_MLP_parameters = {'n_hidden':0, 'n_neurons':0, 'activation':"relu", 'learning_rate': 0.474, 'input_shape': [X_train_DL.shape[1]]}
...
# Fit the model
history = model.fit(X_train_DL, y_train, epochs = 1000, validation_data=(X_val_DL, y_val), callbacks=[checkpoint_cb, early_stopping_cb], verbose = disp_DL)
...
def build_model(n_hidden=1, n_neurons=30, learning_rate=3e-3):
    model = Sequential()
    model.add(InputLayer(shape=(X_train_DL.shape[1], X_train_DL.shape[2])))
...
# Set the parameters for the network
best_CNN_parameters = {'n_hidden':3, 'n_neurons':57, 'learning_rate': 0.005}
...
# Set the parameters for the network
best_RNN_parameters = {'n_hidden':1, 'n_neurons':53, 'learning_rate': 5.8e-4}
...
# Set the parameters for the network
best_LSTM_parameters = {'n_neurons':92, 'n_lstm_hidden':2, 'neurons_dense': 1, 'dropout_rate': 0, 'n_dense_hidden':2, 'learning_rate': 4.3e-4}
...
# Define the function to create models for the optimization method
def build_model(filters = 4, kernel_size = 3, strides = 4, n_neurons=28, n_gru_hidden = 2, neurons_dense = 1, dropout_rate = 0, n_dense_hidden = 1, learning_rate=6.1e-4):
    model = Sequential()
    model.add(InputLayer(shape=(X_train_DL.shape[1], X_train_DL.shape[2])))
    model.add(Conv1D(filters=filters, kernel_size=kernel_size, strides=strides, padding="valid"))
    if n_gru_hidden == 1:
...
# Set the parameters for the network
best_GRU_parameters = {'filters':4, 'kernel_size':3, 'strides': 4, 'n_neurons': 28, 'n_gru_hidden':2, 'neurons_dense': 1, 'dropout_rate': 0, 'n_dense_hidden': 1, 'learning_rate': 6.1e-4}
...
# Set the parameters for the network
best_WaveNet_parameters = {'dilation_rate':(1,2,4), 'repeat':1, 'learning_rate': 2.8e-4}
...
eTS_ = f'{Model_Name[:3]} & {NRMSE:.2f} & {NDEI:.2f} & {MAPE:.2f} & {Rules}'
...
Simpl_eTS_ = f'{Model_Name[:10]} & {NRMSE:.2f} & {NDEI:.2f} & {MAPE:.2f} & {Rules}'
...
exTS_ = f'{Model_Name[:4]} & {NRMSE:.2f} & {NDEI:.2f} & {MAPE:.2f} & {Rules}'
...
# Define Grid Search parameters
parameters = {'alpha': [0.001, 0.01, 0.1, 0.9], 'beta': [0.001, 0.005, 0.01, 0.1, 0.2], 'lambda1': [0.001, 0.1], 'omega': [100, 10000], 'sigma': [0.1, 0.25, 0.5, 0.75]}
grid = ParameterGrid(parameters)
...
ePL_ = f'{Model_Name[:3]} & {NRMSE:.2f} & {NDEI:.2f} & {MAPE:.2f} & {Rules}'
...
eMG_ = f'{Model_Name[:3]} & {NRMSE:.2f} & {NDEI:.2f} & {MAPE:.2f} & {Rules}'
...
ePL_plus_ = f'{Model_Name[:4]} & {NRMSE:.2f} & {NDEI:.2f} & {MAPE:.2f} & {Rules}'
...
ePL_KRLS_DISCO_ = f'{Model_Name[:14]} & {NRMSE:.2f} & {NDEI:.2f} & {MAPE:.2f} & {Rules}'
...
# Store results to dataframe
param = ', '.join(f"{key.replace('_', '\\_')}: {str(value).replace('_', '\\_')}" for key, value in best_GEN_NTSK_wRLS_params.items()) + ' \\\\'
newrow = pd.DataFrame([[Model_Name, NRMSE, NDEI, MAPE, Rules, param]], columns=columns)
# Store results to dataframe
if results.empty:
    results = newrow.copy()
